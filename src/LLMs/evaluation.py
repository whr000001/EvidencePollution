...
def main():
    ...
    results = json.load(open(f'res/{dataset}_{suffix}.json'))
    ...
    for item, res in zip(data, results):
        ...
        print(res)
        input()
        prediction = get_prediction(res, dataset)
    ...
    acc, f1 = [], []
    for _ in range(10):
        fold_index = json.load(open(f'../../datasets_sampled/fold/{dataset}_{_}.json'))
