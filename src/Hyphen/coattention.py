...
class CoAttention(nn.Module):
    ...
    """This is the class for Hyperbolic Fourier-co-attention mechanism."""
    
    def __init__(self, latent_dim=100, embedding_dim=100, manifold=None):
        ...
        self.c = 1
        self.map_comments = HypAct(manifold, 1, self.c, act=torch.tanh)
        self.map_content = HypAct(manifold, 1, self.c, act=torch.tanh)
        self.fourier = True
    ...
    def forward(self, sentence_rep, comment_rep):
        ...
        if self.fourier:
            ...
            sentence_rep = torch.fft.fft2(sentence_rep).real.float()

            comment_rep = logmap0(comment_rep, c=curv)
            comment_rep = torch.fft.fft2(comment_rep).real.float()
        ...
        # Hs = torch.tanh(torch.matmul(self.Ws, sentence_rep_trans) +
        # torch.matmul(torch.matmul(self.Wc, comment_rep_trans), L))
        ...
        # Hc = torch.tanh(torch.matmul(self.Wc, comment_rep_trans)+
        # torch.matmul(torch.matmul(self.Ws, sentence_rep_trans), L_trans))
        ...
        co_sc = torch.squeeze(co_sc, dim=1)
