...
class MyDataset(Dataset):
    def __init__(self, dataset_name, ensemble_type):
        self.max_length = 256
        self.tokenizer = AutoTokenizer.from_pretrained('/data02/whr/resources/DeBERTa-v3', local_files_only=True)
        data = json.load(open(f'../../datasets_{ensemble_type}/{dataset_name}.json'))
        self.data = []
        for item in tqdm(data, desc=f'{dataset_name}', leave=False):
...
def main():
    dataset = MyDataset('twitter15', 'ensemble_all')
