...
from ensemble_dataset import NewsDataset, MySampler
...
def main():
    dataset = NewsDataset(dataset_name, pollution, device)
    acc, f1 = [], []
    for _ in range(10):
        fold_index = json.load(open(f'../../datasets_sampled/fold/{dataset_name}_{_}.json'))
        model = MyModel(num_class=dataset.num_class, device=device).to(device)
