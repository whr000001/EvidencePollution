...
from polluted_dataset import NewsDataset, MySampler, my_collate_fn
...
def updating(fold_index, dataset):
    ...
    train_loader = DataLoader(dataset, batch_size=5,
                              collate_fn=lambda x: my_collate_fn(x, device), sampler=train_sampler)
    test_loader = DataLoader(dataset, batch_size=16,
                             collate_fn=lambda x: my_collate_fn(x, device), sampler=test_sampler)
    model = MyModel(num_class=train_loader.dataset.num_class, device=device).to(device)
