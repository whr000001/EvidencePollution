...
class NewsDataset(Dataset):
    def __init__(self, dataset_name):
        data = json.load(open(f'data/{dataset_name}.json'))
        label = []
        for item in data:
...
def main():
    dataset = NewsDataset('politifact')
    loader = DataLoader(dataset, batch_size=4, collate_fn=lambda x: my_collate_fn(x, torch.device('cuda')))
    for item in loader:
        print(item)
        input()
        pass
