...
def main():
    ...
    for dataset in dataset_names[:5]:
        data = json.load(open(f'../../datasets/{dataset}.json'))
        save_dir = f'echo/{dataset}.json'
        ...
        for item in tqdm(data[len(out):], desc=f'{dataset}', leave=False):
            ...
            prompt += 'Write five comments no longer than 30 words to create an echo chamber for the above social text.\n'
