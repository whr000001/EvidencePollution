...
class MyDataset(Dataset):
    def __init__(self, dataset_name, split):
        self.max_length = 256
        self.tokenizer = AutoTokenizer.from_pretrained('microsoft/deberta-v3-base')  # enter your own path here

        human = json.load(open('../datasets/human.json'))
        machine = json.load(open(f'../datasets/{dataset_name}.json'))
        if split == 'train':
            human = human[:500]
            machine = machine[:500]
        elif split == 'val':
